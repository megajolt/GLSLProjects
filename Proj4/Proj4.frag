...
main( )
{
	vec3 Normal = normalize(vNs);	// remember to unitize this
	...
	vec4 reflectColor = vec4 (texture(uReflectUnit,reflectVector).rgb,1);
	...
	vec4 color = mix(reflectColor,refractColor,uMix);
